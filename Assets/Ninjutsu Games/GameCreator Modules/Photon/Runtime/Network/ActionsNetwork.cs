...
namespace NJG.PUN
{
    ...
    public class ActionsNetwork : MonoBehaviourPun, IMatchmakingCallbacks
    {
            ...
            if (photonView.IsMine || photonView.IsSceneView)
            {
                for (int i = 0, imax = actions.Count; i < imax; i++)
                {
                    ...
                    rpc.actions.onExecute.AddListener((go) => 
                    {
                        ...
                        //rpc.lastInvoker = go;
                        if(photonView.IsSceneView && go.name.StartsWith("==>"))
                        {
                            go.name = string.Format("{0}", go.name.Replace("==>", string.Empty));
                            return;
                        }
        ...
        private void OnActionsExecute(int index)
        {
            ...
            //Debug.LogWarningFormat("[ActionsNetwork] OnActionsExecute index: {0} name: {4} isSceneView: {1} isMine: {2}, isExecuting: {3}"
            //    , index, photonView.IsSceneView, photonView.IsMine, rpc.actions.actionsList.isExecuting, rpc.actions.gameObject.name);
            ...
            {
                photonView.RPC(RPC, rpc.targetPlayer.GetPhotonPlayer(gameObject), index);
            }
            else
            {
                ...
                photonView.RPC(RPC, rpc.targets, index);
            }
        }
        ...
        public virtual void APRPC(int index, PhotonMessageInfo info)
        {
            ...
            //Debug.LogWarningFormat("[ActionsNetwork] RPC index: {0} name: {1} isExecuting: {2} isSceneView: {3} isMine: {4} sender: {5} lastSender: {6}", 
            //    index, rpc.actions.gameObject.name, rpc.actions.actionsList.isExecuting, photonView.IsSceneView, info.photonView.IsMine, info.Sender, rpc.lastSender);
