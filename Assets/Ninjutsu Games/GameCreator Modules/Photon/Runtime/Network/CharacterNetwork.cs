...
{
    [AddComponentMenu("")]
    [RequireComponent(typeof(PhotonView), typeof(Character))]
    public class CharacterNetwork : ObjectNetwork, IPunObservable, IPunInstantiateMagicCallback
    {
        ...
        public bool syncAttachments = false;
        public bool networkCulling = false;
        ...
        private readonly Hashtable EMPTY_HASHTABLE = new Hashtable();
        ...
        public void SetupPhotonView()
        {
            ...
            if (!photonView.ObservedComponents.Contains(this))
            {
                ...
                else photonView.ObservedComponents.Insert(0, this);
            }
        }
#endif
        ...
        private void Start()
        {
            if (!initialized)
            {
                ...
                if (networkCulling && photonView.IsMine && PhotonNetwork.InRoom)
                {
                    ...
                    {
                        this.photonView.Group = CullArea.FIRST_GROUP_ID;
                ...
                if (PhotonNetwork.InRoom && !photonView.IsMine && character is PlayerCharacter)
                {
                    ...
                    if (Locomotion.faceDirection != CharacterLocomotion.FACE_DIRECTION.Target)
                    {
                        ...
                        Locomotion.faceDirectionTarget.target = GameCreator.Core.TargetPosition.Target.Invoker;
                    }
        ...
        private void FormatName()
        {
            ...
            bool master = player == null ? false : player.IsMasterClient;
        ...
        private void Update()
        {
            ...
            if (photonView.IsMine)
            {
                ...
                if (Time.time > lastUpdate)
                {
                    ...
                    if (locomotion.syncAngularSpeed && currentAngularSpeed != lastAngularSpeed)
                    {
            ...
            if (!photonView.IsMine)
            {
                Locomotion.faceDirectionTarget.offset = direction;
...
#region Network Culling
        ...
        private bool HaveActiveCellsChanged()
        {
            ...
            this.activeCells = CullArea.GetActiveCells(transform.position);
            ...
            while (this.activeCells.Count <= CullArea.NumberOfSubdivisions)
            {
                this.activeCells.Add(CullArea.FIRST_GROUP_ID);
            }
            ...
            if (this.activeCells[CullArea.NumberOfSubdivisions] != this.previousActiveCells[CullArea.NumberOfSubdivisions])
            {
                return true;
            }
            ...
            Vector2 controls, Vector2 speed, Vector2 gravity, Vector2 jump, int faceDirection, Hashtable attachments)
        {
            ...
            lcm.canRun = controls.x == 1;
            lcm.canJump = controls.y == 1;
        ...
        public override void OnPlayerEnteredRoom(Player player)
        {
            if (photonView.IsMine && gameObject.activeSelf)
            {
                ...
                photonView.RPC(RPG_CHARACTER, player, transform.position, lcm.canUseNavigationMesh, currentControls, new Vector2(Locomotion.runSpeed, currentAngularSpeed), 
                    currentGravity, currentJump, (int)lcm.faceDirection, syncAttachments ? SerializeAttachments() : EMPTY_HASHTABLE);
            }
        }

        public const int MASK_Stats = 1;
        public const int MASK_Position = 2;
        public const int MASK_Direction = 4;
        //public const int MASK_Rotation = 8;
        public const int MASK_Speed = 8;
        public const int MASK_Run = 16;
        public const int MASK_Jump = 32;
        public const int MASK_PatrolPoint = 64;
        public const int MASK_Home = 128;
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            if (stream.IsWriting)
            {
                ...
                direction = Locomotion.currentLocomotionSystem.movementDirection;
                //storedPosition = transform.position;

                if(locomotion.syncPosition && 
                    !lastDirection.AlmostEquals(direction, PhotonNetwork.PrecisionForVectorSynchronization))
                {
                    sendDirection = true;
                ...
                if (locomotion.syncRunSpeed && lastSpeed != Locomotion.runSpeed)
                {
                    sendSpeed = true;
                ...
                if (sendJump) stream.SendNext(Locomotion.canJump);
                    ...
                    if (sendSpeed) stream.SendNext(Locomotion.runSpeed);
                    //if (sendRotation) stream.SendNext(transform.eulerAngles.y);
                    if (sendDirection) stream.SendNext(direction);
            ...
            else
            {
                ...
                if (receivedPosition)
                {
                    ...
                    {
                        float lag = Mathf.Abs((float)(PhotonNetwork.Time - lastReceivedTime));
                        this.targetPosition += this.direction * lag;
                        this.targetDistance = Vector3.Distance(transform.position, this.targetPosition);
...
#else
                        if (character.enabled && !Locomotion.currentLocomotionSystem.isDashing && character.IsGrounded())
                        {
                            useSmoothPosition = false;
                            Locomotion.SetTarget(targetPosition, null, STOP_THRESHOLD);
                        }
            ...
            if (networkCulling)
            {
                ...
                {
                    this.orderIndex = (++this.orderIndex % CullArea.SUBDIVISION_FIRST_LEVEL_ORDER.Length);
                    this.photonView.Group = this.activeCells[CullArea.SUBDIVISION_FIRST_LEVEL_ORDER[this.orderIndex]];
                }
                else if (CullArea.NumberOfSubdivisions == 2)
                {
                    this.orderIndex = (++this.orderIndex % CullArea.SUBDIVISION_SECOND_LEVEL_ORDER.Length);
                    this.photonView.Group = this.activeCells[CullArea.SUBDIVISION_SECOND_LEVEL_ORDER[this.orderIndex]];
                }
                else if (CullArea.NumberOfSubdivisions == 3)
                {
                    this.orderIndex = (++this.orderIndex % CullArea.SUBDIVISION_THIRD_LEVEL_ORDER.Length);
                    this.photonView.Group = this.activeCells[CullArea.SUBDIVISION_THIRD_LEVEL_ORDER[this.orderIndex]];
                }
            }
        }
