namespace GameCreator.Core
{
    ...
    public class ActionPhotonChangeText : IAction
    {
        ...
        private void UpdateText()
        {
            if (this.text != null)
            {
                this.text.text = PhotonVariableParser.Parse(this.content, gameObject, out needsMonoUpdate);
            }
        ...
        {
            return string.Format(NODE_TITLE, this.text == null ? "(none)" : this.text.gameObject.name);
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            this.toggle = EditorGUILayout.Foldout(this.toggle, "Reference", true);
            EditorGUI.indentLevel--;
            ...
            if (this.toggle)
            {
                ...
                EditorGUILayout.HelpBox("TIP: For Player's related data you are allowed to use:\n1. # ID (0,1,2 etc) of the player.\n2. Use 'local' to get your player.\n3. Use 'this' to get the Player from the attached PhotonView.", MessageType.Info);
                ...
                EditorGUILayout.HelpBox("TIP: For Time related data you are allowed to set the format after delimiter '|' like hh':'mm':'ss (Hours, Minutes, Seconds)\n\nNote that will only work when the Start Room Timer action is called", MessageType.Info);
