namespace GameCreator.Core
{
    ...
    public class IgniterOnPhotonRoomEvents : Igniter, IInRoomCallbacks
    {
        ...
        {
            if (eventType == PhotonEvent.OnPlayerEnteredRoom) ExecuteTrigger(gameObject);
        }

        public void OnPlayerLeftRoom(Player otherPlayer)
        {
            if (eventType == PhotonEvent.OnPlayerLeftRoom) ExecuteTrigger(gameObject);
        }
        ...
        {
            if (eventType == PhotonEvent.OnMasterClientSwitched) ExecuteTrigger(gameObject);
        }
    }
}